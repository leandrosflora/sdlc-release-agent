"""sdlc-release-agent: operational agent implementing agent_role == "release".
...
ACTIONS: Dict[str, Callable[[dict], ActionResult]] = {
    ...
    "artifact.publish": _stub("artifact.publish", "publish a build artifact (CI only)"),
    "deployment.nonprod": _stub("deployment.nonprod", "trigger a non-production deployment"),
    "production.deploy": _stub("production.deploy", "trigger a production deployment (Release gate)")
}
